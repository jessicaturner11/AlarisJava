...
public class AlarisTests {
	...
	/*	Field [] fields = frame.getClass().getDeclaredFields();
		for(Field f: fields){
			...
			/*No annontations were found for any fields
			Annotation [] annontations = f.getDeclaredAnnotations();
	...
	public void mappingTests(){
		...
		//onStep((Initialise,Press,OnOff)) & behaviourMap(OnOff,(S_SwitchOn, I_ClearSetup))
		OnOff.click();
		
		//Thread.dumpStack();
		//OnOff.target().getAction().getValue(key)
		
		//ActionListener[] listeners = OnOff.target().getActionListeners();
	...
	/*	try{
			Class alaris_class = Class.forName("AlarisDummyMVN.jdt11.Alaris");
			System.out.println(alaris_class.getName());
			Method m = alaris_class.getEnclosingMethod();
			if(m!=null){
		...
		assertEquals("ClearSetup", frame.getMode());
		//onStep((OnOff,Observe,Alarm)) & behaviourMap(Alarm,())
		assertEquals("SwitchOn()", frame.getFunction());
		assertEquals("ClearSetup", frame.getMode());
		//onStep((Alarm,Observe,AlarmLight)) & behaviourMap(AlarmLight,())
		assertEquals("SwitchOn()", frame.getFunction());
		assertEquals("ClearSetup", frame.getMode());
		//onStep((AlarmLight,Observe,AlarmLight)) & behaviourMap(AlarmLight,())
		assertEquals("SwitchOn()", frame.getFunction());
		assertEquals("ClearSetup", frame.getMode());
		//onStep((AlarmLight,Observe,Display)) & behaviourMap(Display,(I_SelectListItem))
		assertEquals("SwitchOn()", frame.getFunction());
		assertEquals("ClearSetup", frame.getMode());
		//onStep((Display,Press,Button1)) & behaviourMap(Button1,(I_ConfirmProfile, S_ClearSetup))
		Button1.click();
		assertEquals("ClearSetup()", frame.getFunction());
		assertEquals("ConfirmProfile", frame.getMode());
		//onStep((Button1,Press,Button3)) & behaviourMap(Button3,(I_Profile))
		Button3.click();
		assertEquals("Profile", frame.getMode());
		//onStep((Button3,Press,Down)) & behaviourMap(Down,(S_DownProfileOne))
		Down.click();
		assertEquals("DownProfileOne()",frame.getFunction());
		assertEquals("Profile", frame.getMode());
		//onStep((Down,Press,Down)) & behaviourMap(Down,(S_DownProfileOne))
		Down.click();
		assertEquals("DownProfileOne()",frame.getFunction());
		assertEquals("Profile", frame.getMode());
		//onStep((Down,Press,Button1)) & behaviourMap(Button1,(I_ConfirmProfile))
		Button1.click();
		assertEquals("ConfirmProfile", frame.getMode());
		//onStep((Button1,Press,Button1)) & behaviourMap(Button1,(I_SelectCategory, S_SetProfile))
		Button1.click();
		assertEquals("SetProfile()", frame.getFunction());
		assertEquals("SelectDrugCategory", frame.getMode());
		//onStep((Button1,Press,Down)) & behaviourMap(Down,(S_DownCategoryOne))
		Down.click();
		assertEquals("DownCategoryOne()", frame.getFunction());
		//onStep((Down,Press,Button1)) & behaviourMap(Button1,(I_SelectDrug, S_SetDrugCategory))
		Button1.click();
		assertEquals("SetDrugCategory()", frame.getFunction());
		assertEquals("SelectDrug",frame.getMode());
		//onStep((Button1,Press,Down)) & behaviourMap(Down,(S_DownDrugsOne))
		Down.click();
		assertEquals("DownDrugsOne()",frame.getFunction());
		//onStep((Down,Press,Down)) & behaviourMap(Down,(S_DownDrugsOne))
		Down.click();
		assertEquals("DownDrugsOne()",frame.getFunction());
		//onStep((Down,Press,Down)) & behaviourMap(Down,(S_DownDrugsOne))
		Down.click();
		assertEquals("DownDrugsOne()",frame.getFunction());
		//onStep((Down,Press,Button1)) & behaviourMap(Button1,(I_ConfirmRate, S_SetDrug))
		Button1.click();
		assertEquals("SetDrug()", frame.getFunction());
		assertEquals("ConfirmRate", frame.getMode());
		//onStep((Button1,Press,Button2)) & behaviourMap(Button2,(I_RateOnHold, S_SetRate, S_ChangeReset))
		Button2.click();
		assertEquals("SetRate() ChangeReset()",frame.getFunction());
		assertEquals("RateOnHold",frame.getMode());
		//onStep((Button2,Press,Button2)) & behaviourMap(Button2,(I_VTBI, S_ChangeReset))
		Button2.click();
		assertEquals("ChangeReset()", frame.getFunction());
		assertEquals("VTBI", frame.getMode());
		//onStep((Button2,Press,Button2)) & behaviourMap(Button2,(I_VTBIBags))
		Button2.click();
		assertEquals("VTBIBags", frame.getMode());
		//onStep((Button2,Press,Up)) & behaviourMap(Up,(S_UpVTBIBagsOne))
		Up.click();
		assertEquals("UpVTBIBagsOne()", frame.getFunction());
		//onStep((Up,Press,Button1)) & behaviourMap(Button1,(I_VTBI, S_SetVTBIBags, S_ChangeReset))
		Button1.click();
		assertEquals("SetVTBIBags() ChangeReset()", frame.getFunction());
		assertEquals("VTBI", frame.getMode());
		//onStep((Button1,Press,Button1)) & behaviourMap(Button1,(I_EndRate, S_SetVTBI, S_ChangeReset))
		Button1.click();
		assertEquals("SetVTBI() ChangeReset()", frame.getFunction());
		assertEquals("EndRate", frame.getMode());
		//onStep((Button1,Press,Up)) & behaviourMap(Up,(S_UpEndRateOne))
		Up.click();
		assertEquals("UpEndRateOne()", frame.getFunction());
		//onStep((Up,Press,Button1)) & behaviourMap(Button1,(I_RateOnHold))
		Button1.click();
		assertEquals("RateOnHold", frame.getMode());
		//onStep((Button1,Press,Run)) & behaviourMap(Run,(I_RateInfusing, S_Infusing))
		Run.click();
		assertEquals("Infusing()", frame.getFunction());
		assertEquals("RateInfusing", frame.getMode());
		//onStep((Run,Observe,Display)) & behaviourMap(Display,(S_IncreaseRate, S_DecreaseRate, S_IncreaseRateDouble, S_DecreaseRateDouble))
		assertEquals("Infusing()", frame.getFunction());
		assertEquals("RateInfusing", frame.getMode());
		//onStep((Display,Observe,RunLight)) & behaviourMap(RunLight,(S_Infusing))
		assertEquals("Infusing()", frame.getFunction());
		assertEquals("RateInfusing", frame.getMode());
	}
	...
	private void threadMethod(){
		...
		for(Entry<Thread, StackTraceElement[]> entry: map.entrySet()){
			//if(entry.getKey().getName().equals("Alaris")){
				System.out.println("-------------------------");
	...
	private String getAlarisMethod(){
		Map<Thread, StackTraceElement[]> threads = Thread.getAllStackTraces();
		StackTraceElement[] current = new StackTraceElement[0];
		for(Map.Entry<Thread, StackTraceElement[]> entry : threads.entrySet()){
			/*System.out.println(entry.getKey().getName());
			if(entry.getValue().length > 0){
				System.out.println(entry.getValue()[0].getMethodName());
			}
			if(entry.getKey().getName().equals("Alaris")){
		...
		if(current!=null&&current.length>0){
			...
			return current[0].getMethodName();
		} else {
			return null;
		}
	}*/
